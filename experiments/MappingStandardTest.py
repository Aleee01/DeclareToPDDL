...
domain_output = "domain7_10.pddl"
problem_output = "problem7_10.pddl"
...
activities_csv = "activities_10.csv"
...
for act in activities:
    if not actions[act]["eff"]:
        # Se non ha effetti già definiti dai vincoli
        actions[act]["eff"].add(pred("done", act))
...
for _, row in df.iterrows():
    if str(row.get("template")).strip().lower() == "notcoexistence":
        a = norm(row.get("activity_a"))
        b = norm(row.get("activity_b"))
        if a and b:
            notcoex_pairs.add(tuple(sorted((a, b))))


exclude_from_goal = set()

for a, b in notcoex_pairs:
    # regola semplice e stabile:
    # tieni la prima alfabeticamente, escludi la seconda
    exclude_from_goal.add(b)
...
for a in activities:
    ...
    # not coexistence: togli una delle due
    if a_norm in exclude_from_goal:
        continue
