...
def automaton_notresponse(a, b, idx, all_acts):
    s0 = f"notresp_{a}_{b}_s0_{idx}"
    s1 = f"notresp_{a}_{b}_s1_{idx}"
    sink = f"notresp_{a}_{b}_sink_{idx}"
    ...
    for act in all_acts:
        ...
        if act != b:
            transitions.append((s1, act, s1))
    return {
        "name": f"notresponse_{a}_{b}_{idx}",
        "states": [s0, s1, sink],
        "init": s0,
        "final": [s0, s1],
        "transitions": transitions,
...
def automaton_notchainresp(a, b, idx, all_acts):
    s0 = f"notchainresp_{a}_{b}_s0_{idx}"
    s1 = f"notchainresp_{a}_{b}_s1_{idx}"
    sink = f"notchainresp_{a}_{b}_sink_{idx}"
    ...
    for act in all_acts:
        ...
        if act not in (a, b):
            transitions.append((s1, act, s0))
    return {
        "name": f"notchainresponse_{a}_{b}_{idx}",
        "states": [s0, s1, sink],
        "init": s0,
        "final": [s0, s1],
        "transitions": transitions,
...
BUILDERS = {
    ...
    "notresponse": automaton_notresponse,
    "notprecedence": automaton_notresponse,
    "notsuccession": automaton_notresponse,
    "notchainresponse": automaton_notchainresp,
    "notchainprecedence": automaton_notchainresp,
    "notchainsuccession": automaton_notchainresp,
...
def main():
    ...
    with open(ACTIVITIES_CSV, newline="", encoding="utf-8") as f:
        ...
        if "activity" not in reader.fieldnames:
            print("❌ Colonna 'activity' mancante in activities CSV")
            return

        for row in reader:
            act = row["activity"].strip().replace(" ", "_")
            if act:
                activities_set.add(act)
    ...
    with open(CSV_PATH, newline="", encoding="utf-8") as f:
        ...
        required_cols = {"template", "activity_a"}
        if not required_cols.issubset(reader.fieldnames):
            print(f"❌ CSV vincoli deve contenere le colonne: {required_cols}")
            return

        for row in reader:
            rows.append(row)

    print(f"📄 Vincoli letti: {len(rows)}")
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        b = None
        if "activity_b" in row and row["activity_b"]:
            b = row["activity_b"].strip().replace(" ", "_")
            if b == "":
                b = None
        ...
        try:
            ...
            print(f"❌ Errore costruzione automa riga {idx}: {e}")
            continue
